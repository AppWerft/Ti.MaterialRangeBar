 ...
 * {@link android.widget.SeekBar} with discrete values. Whereas the thumb for
 * the SeekBar can be dragged to any position in the bar, the RangeBar only
 * allows its thumbs to be dragged to discrete positions (denoted by tick marks)
 * in the bar. When released, a RangeBar thumb will snap to the nearest tick
 * mark. This version is forked from edmodo range bar
 * https://github.com/edmodo/range-bar.git Clients of the RangeBar can attach a
 * {@link com.appyvet.rangebar.RangeBar.OnRangeBarChangeListener} to be notified
...
public class RangeBar extends View {
	...
	protected void onSizeChanged(int w, int h, int oldw, int oldh) {
		...
		if (mIsRangeBar) {
			...
			mLeftThumb.init(ctx, yPos, expandedPinRadius, mPinColor,
					mTextColor, mCircleSize, mCircleColor, mMinPinFont,
					mMaxPinFont, mArePinsTemporary);
		}
		mRightThumb = new PinView(ctx);
		mRightThumb.setFormatter(mFormatter);
		mRightThumb.init(ctx, yPos, expandedPinRadius, mPinColor, mTextColor,
				mCircleSize, mCircleColor, mMinPinFont, mMaxPinFont,
				mArePinsTemporary);
		...
		mBar = new Bar(ctx, marginLeft, yPos, barLength, mTickCount,
				mTickHeightDP, mTickColor, mBarWeight, mBarColor);
		...
		final int newLeftIndex = mIsRangeBar ? mBar
				.getNearestTickIndex(mLeftThumb) : 0;
	 ...
	 *            the RangeBar pin text notification listener; null to remove
	...
	public void setTickStart(float tickStart) {
		...
		if (isValidTickCount(tickCount)) {
			...
			if (mFirstSetTickCount) {
				...
				if (mListener != null) {
					mListener.onRangeChangeListener(this, mLeftIndex,
							mRightIndex, getPinValue(mLeftIndex),
	 ...
	 *
	 * @param pinColor
	 *            Integer specifying the color of the pin.
	 */
	 ...
	 *
	 * @param textColor
	 *            Integer specifying the color of the text in the pin.
	 */
	 ...
	 *
	 * @param isRangeBar
	 *            Boolean - true sets it to rangebar, false to seekbar.
	 */
	 ...
	 *
	 * @param arePinsTemporary
	 *            Boolean - true if pins shoudl dissapear after released, false
	 *            to stay drawn
	 */
	 ...
	 *
	 * @param tickColor
	 *            Integer specifying the color of the ticks.
	 */
	...
	/**
	 * Sets the location of the pins according by the supplied index. Numbered
	 * from 0 to mTickCount - 1 from the left.
	 *
	 * @param leftPinIndex
	 *            Integer specifying the index of the left pin
	 * @param rightPinIndex
	 *            Integer specifying the index of the right pin
	 */
	...
	/**
	 * Sets the location of pin according by the supplied index. Numbered from 0
	 * to mTickCount - 1 from the left.
	 *
	 * @param pinIndex
	 *            Integer specifying the index of the seek pin
	 */
	 ...
	 *
	 * @param leftPinValue
	 *            Float specifying the index of the left pin
	 * @param rightPinValue
	 *            Float specifying the index of the right pin
	 */
	 ...
	 *
	 * @param pinValue
	 *            Float specifying the value of the pin
	 */
	 ...
	 */
	private void rangeBarInit(Context context) {
		// TODO tick value map
		if (mTickMap == null) {
		...
		try {
			...
			// attributes.
			final float tickStart = ta.getFloat(R.styleable.RangeBar_tickStart,
					DEFAULT_TICK_START);
			final float tickEnd = ta.getFloat(R.styleable.RangeBar_tickEnd,
					DEFAULT_TICK_END);
			final float tickInterval = ta.getFloat(
					R.styleable.RangeBar_tickInterval, DEFAULT_TICK_INTERVAL);
			...
			mCircleSize = ta.getDimension(R.styleable.RangeBar_selectorSize,
					TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
							DEFAULT_CIRCLE_SIZE_DP, getResources()
									.getDisplayMetrics()));
			...
			mExpandedPinRadius = ta.getDimension(
					R.styleable.RangeBar_pinRadius, TypedValue.applyDimension(
							...
							DEFAULT_EXPANDED_PIN_RADIUS_DP, getResources()
									.getDisplayMetrics()));
			...
			mBarPaddingBottom = ta.getDimension(
					R.styleable.RangeBar_rangeBarPaddingBottom, TypedValue
							.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
									DEFAULT_BAR_PADDING_BOTTOM_DP,
									getResources().getDisplayMetrics()));
			mIsRangeBar = ta.getBoolean(R.styleable.RangeBar_rangeBar, true);
			mArePinsTemporary = ta.getBoolean(
					R.styleable.RangeBar_temporaryPins, true);
	...
	private void createBar() {
		mBar = new Bar(getContext(), getMarginLeft(), getYPos(),
				getBarLength(), mTickCount, mTickHeightDP, mTickColor,
	...
	private void createPins() {
		...
		if (mIsRangeBar) {
			mLeftThumb = new PinView(ctx);
			mLeftThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize,
					mCircleColor, mMinPinFont, mMaxPinFont, false);
		}
		mRightThumb = new PinView(ctx);
		mRightThumb.init(ctx, yPos, 0, mPinColor, mTextColor, mCircleSize,
				mCircleColor, mMinPinFont, mMaxPinFont, false);
	 ...
	 *
	 * @param leftThumbValue
	 *            Float specifying the left thumb value.
	 * @param rightThumbValue
	 *            Float specifying the right thumb value.
	 * @return boolean If the index is out of range.
	 */
	...
	private void onActionUp(float x, float y) {
		if (mIsRangeBar && mLeftThumb.isPressed()) {
			...
			float leftThumbXDistance = mIsRangeBar ? Math.abs(mLeftThumb.getX()
					- x) : 0;
	...
	private void onActionMove(float x) {
		...
		// Get the updated nearest tick marks for each thumb.
		int newLeftIndex = mIsRangeBar ? mBar.getNearestTickIndex(mLeftThumb)
				: 0;
	...
	private void releasePin(final PinView thumb) {
		...
		if (mArePinsTemporary) {
			ValueAnimator animator = ValueAnimator.ofFloat(mExpandedPinRadius,
					0);
			animator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
	...
	/**
	 * Set the value on the thumb pin, either from map or calculated from the
	 * tick intervals Integer check to format decimals as whole numbers
	 *
	 * @param tickIndex
	...
	private String getPinValue(int tickIndex) {
		...
		float tickValue = (tickIndex == (mTickCount - 1)) ? mTickEnd
